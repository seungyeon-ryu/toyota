(function($){
		...
		return {
			...
			mainVisual : function(){
				...
				$mainVisual.on('beforeChange', function(event, slick, currentSlide, nextSlide){
					...
					$(slick.$slides[currentSlide]).removeClass('mv-start').addClass('mv-stop');
	...
	newriver.headerAction = function(_){
		...
		$(document).on('scroll touchmove', function(e){
			...
			//nowScrollTop <= 0 ? prevScrollTop = 0 : prevScrollTop = nowScrollTop;
		...
		$('#trgAside').on('click', function(){
			var delay = 100;
			if(!flag){
				$('body').addClass('asideOpen').css('overflow','hidden');
				//_.$gnbdepth.addClass('on');

				// _.$gnbdepth.delay(300).queue(function(){
				// 	_.$gnbdepth.each(function(index, item){
				// 		setTimeout(function(){
				// 			$(item).addClass('on');
				// 		}, index * delay);
				// 	});
				...
				$('body').removeClass('asideOpen').css('overflow','auto');
				_.$gnbdepth.removeClass('open'); //.removeClass('on');
				flag = false;

				// var _max = _.$gnbdepth.length - 1;//마지막
				// var _idx = 0; //마지막을 체크하기 위한 변수
